...
function calcShare(interval) {
    interval.dayShare = Math.floor((interval.day * 100) / dayShare);
    interval.weekShare = Math.floor((interval.week * 100) / weekShare);
    interval.monthShare = Math.floor((interval.month * 100) / monthShare);
    interval.totalShare = Math.floor((interval.total * 100) / totalShare);
    return interval
}
...
function calcDurations(incoming) {
    ...
    for (let i = 0; i < length / 2; i++) {
        ...
        newLogs.push({ from: logs[0].at, duration });
        logs.shift()
        logs.shift();
    }
    return newLogs;
    ...
    if (query) {
        ...
        <div class="bottom">
          <div class="stat">
            ...
            <span class="value">${intr.dayShare}% . ${parseTime(intr.day)}</span>
          </div>
          <div class="stat">
            <span class="time">Last week</span>
            <span class="value">${intr.weekShare}% . ${parseTime(intr.week)}</span>
          </div>
          <div class="stat">
            <span class="time">Last month</span>
            <span class="value">${intr.monthShare}% . ${parseTime(intr.month)}</span>
          </div>
          <div class="stat">
            <span class="time">Total</span>
            <span class="value">${intr.totalShare}% . ${parseTime(intr.total)}</span>
          </div>
        </div>
        <h3>Logs</h3>
        ...
        if (people.length === querySnapshot.size) {
            ...
            if (people.length) {
                ...
                id('top-total').innerHTML = `
                    ...
                    <span><i>last week</i> ${Math.floor(weekShare / hour)}h<h</span> . 
                    <span><i>last month</i> ${Math.floor(monthShare / hour)}h<h</span>`
            }
            people.sort((x, y) => {
            ...
            people.forEach(person => {
                ...
                <div class="text">
                <div class="name">${person.name}</div>
                <div class="des">${Math.floor(person.interval.totalShare)}% . ${parseTime(person.interval.total)}</div>
                </div>
